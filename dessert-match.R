...
#read my own list
mydessert <- read_csv(here("favorite_desserts.csv"))

mydessert$Favorite_dessert[1]

#try to find my dessert in the iconic list

comparison0 <- dessert_listing %>% dplyr::filter(dessert=="pancakes")

comparison <- "pancakes" %in% dessert_listing$dessert

inner_join(mydessert, iconic_desserts, join_by(Favorite_dessert == dessert))
